...
test.describe('test session', () => {
  ...
  test('sign in / sign out', async ({ page }) => {
    ...
    await expect(page.locator(`text="${data.buttons.signOut.name}"`)).not.toBeVisible();
    ...
    await expect(page.locator(`text="${data.links.signIn.name}"`)).not.toBeVisible();
    await expect(page.locator(`text="${data.links.signUp.name}"`)).not.toBeVisible();
